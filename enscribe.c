...
#define BUZZGAMMA (1.9)
#define HISSGAMMA (2.4)
#define EXPGAMMA  (2.4)
...
float rotate=-90;
float gain=0;
float default_gain_buzz=1.0;	//     0dB
float default_gain_hiss=5.6; 	// ~ +15dB
int use_default_gain=1;
...
//	printf("Usage:  enscribe [switches] [-t text] input_image output_audio\n");
	...
	printf("         g                 greyscale (default)\n");
	printf("         m                 monochrome, ugly\n");
	printf("         rc                red and cyan\n");
	...
	printf("\n");
	printf("  -rotate=[degrees]     Rotate image counter clockwise; 0 degrees orientation\n");
	printf("                           is left side of an image towards low frequencies\n");
	printf("                           (thus for vertical waterfall, like it is in Baudline);\n");
	printf("                           For horizontal left-to-right waterfall set degrees\n");
	printf("                           to -90 (default).\n");
	printf("\n");
...
//	printf("\n");
	...
	printf("\n");
	printf("  -mono, -m              Produce mono ouput audio file (default).\n");
	printf("  -stereo, -s            Produce stereo ouput audio file.\n");
	printf("  -rate=[rate]           Set audio sample rate (44100 default).\n");
	printf("  -gain=[dB]			 Set output audio gain in decibels\n");
	printf("						 (default=+15 for hiss mode and =0 for buzz mode)\n");
	printf("\n");
...
*/
				...
				{
				if (strncmp(argv[i],"-gain=",6)!=0)	// ?
					{
					fprintf(stderr,"Error: You need to specify a gain value\n");
					exit(BAD_ARGS);
					}
				else 
					{
					strncpy(arg,&(argv[i][6]),255);
					gain=pow(10, atof(arg)/20);
					use_default_gain=0;
					}
				}//-rotate
				...
				{
				if (strncmp(argv[i],"-rotate=",8)!=0)	// ?
					{
					fprintf(stderr,"Error: You need to specify a rotation angle, e.g.:\n");
...
//	float dif;
  ...
  if (oversample)
	...
	if (!textmode)
        ...
        im = gdImageRotateInterpolated(im, rotate, 0);
...
fprintf(stderr,"Opening output file\n");
	...
	if (use_default_gain)
		{
			if (randphase)
				gain = default_gain_hiss;
			else
				gain = default_gain_buzz;
		}
	...
	for (y = 0; y < im2->sy; y++)
		...
		for (x = startx; x < stopx; x++)
			...
			switch (colormode)
				{
				case GREYSCALE:
					cl = cr = ( 0.2126 * gdImageRed(im2,c) + 0.7152 * gdImageGreen(im2,c) + 0.0722 * gdImageBlue(im2,c) );
					break;
				case MONOCHROME:
					cl = cr = ( 0.2126 * gdImageRed(im2,c) + 0.7152 * gdImageGreen(im2,c) + 0.0722 * gdImageBlue(im2,c) );
					break;
				...
				case GREENPURPLE:
					...
					break;
				case WHITEBLACK:
					cl = ( 0.2126 * gdImageRed(im2,c) + 0.7152 * gdImageGreen(im2,c) + 0.0722 * gdImageBlue(im2,c) );
					cr = 255 - cl;
					break;
				case EXPERIMENTAL:
					cl = gdImageRed(im2,c);
